...
You can explore the API using Swagger UI. Visit the Swagger documentation at Swagger UI.

Third-party Libraries Used
The project utilizes several third-party libraries to streamline development and enhance its features:

Lombok and Mapstruct: Used to reduce boilerplate code.
Spring Security: Ensures security within the application.
Spring Data MongoDB: Provides connectivity to MongoDB for data storage.
Spring Validations: Enables JSON validation for request data.
Springdoc OpenAPI: Generates API documentation for the project.
Spring Boot AOP: Implements Aspect-Oriented Programming, primarily for logging in this project.
Embedded MongoDB: Utilized for testing purposes.
How to Run the Application
To run the Orders project, follow these steps:

Download Docker: Ensure you have Docker installed on your machine.
Download MongoDB Image: Pull the MongoDB image using the following command: docker pull mongo.
Build the Project: Run mvn clean install in the project directory to build the application.
Build Docker Image: Run docker build . -t kosiara2137/orders:v1 to create a Docker image.
Docker Compose: Navigate to the docker-compose directory and execute docker-compose up to start the application.
Example Requests
